...
// ============================================================
//  Couleur d'accent native (macOS + Windows) — dynamique
// ============================================================
...
fn darken_hex(hex: &str, factor: f64) -> String {
    ...
    let r = (parse(&h[0..2]) as f64 * (1.0 - factor)) as u8;
    let g = (parse(&h[2..4]) as f64 * (1.0 - factor)) as u8;
    let b = (parse(&h[4..6]) as f64 * (1.0 - factor)) as u8;
    format!("#{r:02x}{g:02x}{b:02x}")
}

fn make_accent_pair(accent: &str) -> AccentPair {
    AccentPair {
        accent: accent.to_string(),
        accent_hover: darken_hex(accent, 0.10),
    }
}

// Lecture de la couleur d'accent systeme :
// macOS  → `defaults read -g AppleAccentColor` (entier 0-6)
// Windows → registre HKCU\…\Explorer\Accent\AccentColorMenu (DWORD ABGR)
// Linux  → fallback bleu

#[cfg(target_os = "macos")]
fn read_native_accent() -> String {
    ...
    match idx {
        ...
        5 => "#bf5af2", // Violet
        6 => "#ff375f", // Rose
        _ => "#0a84ff", // Multicolore / défaut = Bleu
    }
    .to_string()
}
...
#[cfg(windows)]
fn read_native_accent() -> String {
    // AccentColorMenu = DWORD stocké en ABGR : R=octet bas, B=octet haut.
    ...
    if let Some(o) = out {
        ...
        for line in s.lines() {
            if line.contains("AccentColorMenu") {
...
#[tauri::command]
pub fn get_accent_color() -> AccentPair {
    make_accent_pair(&read_native_accent())
}

// Surveille la couleur d'accent toutes les 5 s et emet `theme://accent` si elle change.
pub fn start_accent_watcher(app: AppHandle) {
    std::thread::spawn(move || {
        let mut current = read_native_accent();
        loop {
            std::thread::sleep(std::time::Duration::from_secs(5));
            let next = read_native_accent();
            if next != current {
                current = next.clone();
                let _ = app.emit("theme://accent", make_accent_pair(&next));
            }
        }
    });
}
